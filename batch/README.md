...
This example architecture shows how to handle Batch Processing using ECS.
...
The CloudFormation template defaults to using the `DockerImage` at `linuxacademy/ecs-deep-dive:batch`.
...
You can now use this image `<repo>/<image>` as the `DockerImage` parameter in the CloudFormation template.
